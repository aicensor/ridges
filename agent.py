...
GLM_NEW_MODEL_NAME = Model(name="zai-org/GLM-5-FP8", timeout=150)

QWEN_MODEL_NAME = Model(name="Qwen/Qwen3-Coder-Next", timeout=100)

DEEPSEEK_MODEL_NAME = Model(name="deepseek-ai/DeepSeek-R1-0528", timeout=50)
...
- **Do NOT add defensive validation or error handling** unless the problem statement explicitly specifies error types, error messages, or requires specific exception handling
...
2) **Edit**: apply a minimal change using `apply_code_edit`.
3) **Test**: run `run_tests` (or `run_code`) soon after edits.
4) **Finish**: only call `finish` when verification is clean.
...
5) **Required default approach (follow strictly)**
...
### 4) Making Code Changes (minimal, focused)
...
### 5) Debugging (root cause, not symptoms)
...
def format_log(function_name: str, content: str) -> str:
    """Format log message with elapsed time and function name."""
    elapsed_time = time.time() - START_TIME
    # Indent each line of content
    indented_content = "\n                    ".join(content.split("\n"))
    return f"{elapsed_time:.3f}s: [{function_name}]\n\n                    {indented_content}\n"

def Exchange_Model(current_model: str) -> str:
    other_models = [m for m in AGENT_MODELS if m != current_model]
    if other_models:
...
class EnhancedCOT:
    ...
    def _summarize_messages_batch(self, start_idx, end_idx):
        ...
        for _ in range(10):
            try:
                response, _ = EnhancedNetwork.make_request(messages, model=current_model, temperature=0.0, where="summarize_messages_batch")
    ...
    def llm_select_run_command_for_file(cls, file_path: str) -> list[str]:
        ...
        while retry < 10:
            try:
                ...
                raw_text, _ = EnhancedNetwork.make_request(messages, model=QWEN_MODEL_NAME, where="llm_select_run_command_for_file")
...
class SolutionVerifier:
    ...
    def verify_solution(self) -> str:
        ...
        while retry < max_retries:
            try:
                review_result, _ = EnhancedNetwork.make_request(messages, model=selected_model, temperature=0.0, where="verify_solution")
...
class EnhancedNetwork:
    ...
    def _request_next_action_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                raw_text, _ = cls.make_request(messages, model=current_model, temperature=temperature, where="request_next_action_with_retry")
        ...
        for i in range(attempts):
            try:
                ...
                if resp.status_code == 429:
                    ...
                    wait = min(wait * 2, max_wait)  # Exponential backoff
                ...
                model_name = Exchange_Model(model_name)
                time.sleep(1)
                ...
                if status_code == 429:
                    if i >= attempts - 1:
                        raise RuntimeError(f"HTTP ERROR 429: Too many requests for model {model_name} after {attempts} attempts: {e}")
                    model_name = Exchange_Model(model_name)
                    time.sleep(1)
                    continue
    ...
    def fix_json_string_with_llm(cls, json_string: str, attempt: int = 0) -> dict:
        ...
        while retry < 5:
            try:
                response, _ = cls.make_request(messages, model=selected_model, where="fix_json_string_with_llm")
                break
            except Exception:
                retry += 1
                selected_model = Exchange_Model(selected_model)
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def profile_performance(self, file_path: str, profiling_command: List[str], baseline_command: Optional[List[str]] = None, num_runs: int = 3) -> str:
        ...
        try:
            ...
            # Helper function to run profiling multiple times
            ...
            # Show the last run's output (most representative)
    ...
    def rank_fix_confidence(
        ...
        while retry < 3:
            try:
                ...
                response, _ = EnhancedNetwork.make_request(messages, model=QWEN_MODEL_NAME, temperature=0.0, where="rank_fix_confidence")
    ...
    def analyze_edge_cases(self, file_contents: Dict[str, str], target_identifier: str = None) -> str:
        ...
        # Include problem statement if available (but don't hardcode it as required)
        problem_context = ""
        ...
        while retry < 5:
            try:
                analysis, _ = EnhancedNetwork.make_request(messages, model=selected_model, attempt=1, temperature=0.0, where="analyze_edge_case")
    ...
    def generate_test_cases_from_root_cause(self, root_cause_code: str, file_path: str = None, function_name: str = None) -> str:
        ...
        while retry < 10:
            try:
                ...
                test_cases, _ = EnhancedNetwork.make_request(messages, model=selected_model, attempt=1, temperature=0.0, where="generate_test_cases_from_root_cause")
    ...
    try:
        ...
        while retry < max_retries:
            try:
                ...
                response, _ = EnhancedNetwork.make_request(messages, model=selected_model, where="validate_initial_structure_implementation")
...
def generate_function_behaviours(initial_structure: str, problem_statement: str) -> str:
    ...
    while retry < max_retries:
        try:
            ...
            response, _ = EnhancedNetwork.make_request(messages, model=selected_model, timeout=300, where="generate_function_behaviours")
...
def check_not_defined_functions(code: str) -> dict:
    ...
    while retry < max_retries:
        try:
            # Call LLM
            response, _ = EnhancedNetwork.make_request(messages, model=selected_model, timeout=120, where="check_not_defined_functions")
...
def validate_implementation_completeness(code: str) -> str:
    ...
    while retry < max_retries:
        try:
            # Call LLM for validation
            response, _ = EnhancedNetwork.make_request(messages, model=selected_model, timeout=120, where="validate_implementation_completeness")
...
def execute_agent_workflow(
    ...
    for step in range(n_max_steps):
        ...
        # Update cot and solution_verifier if tool_manager is FixTaskEnhancedToolManager
        ...
        while retry < 10:
            try:
                ...
                result, _ = EnhancedNetwork.make_request(
                    ...
                    model=selected_model,
                    where="extract_file_names_using_llm"
                )
...
def select_best_solution(
    ...
    while retry < 10:
        try:
            ...
            result = EnhancedNetwork.make_request(messages, model=selected_model, temperature=0.0, where="select_best_solution")
    ...
    def get_problem_type(problem_statement: str, enhancement: str) -> str:
        ...
        while retry < 3:
            try:
                ...
                response, _ = EnhancedNetwork.make_request(messages, model=selected_model, where="get_problem_type")
    ...
    def run_task():
        ...
        try:
            ...
            if problem_type == PROBLEM_TYPE_FIX:
                result = process_fix_task(problem_statement, enhancement)
            else:
                result = process_create_task(problem_statement, enhancement)
        finally:
            task_completed.set()
