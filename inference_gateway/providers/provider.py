...
class Provider(ABC):
    ...
    async def test_all_inference_models(self):
        ...
        if all(results):
            ...
            logger.warning(f"Failed to test {self.name} inference models: {', '.join([str(model.name) + ' (' + str(model.external_name) + ')' for model, result in zip(self.inference_models, results) if not result])}")
    ...
    async def test_all_embedding_models(self):
        ...
        if all(results):
            ...
            logger.warning(f"Failed to test {self.name} embedding models: {', '.join([model.name for model, result in zip(self.embedding_models, results) if not result])}")
